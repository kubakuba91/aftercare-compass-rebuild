...
import { coverageLabel } from "@/lib/virtual-care";
import type { NormalizedProviderImportRow } from "@/lib/provider-csv-import";
...
export default async function ProviderImportPreviewPage({ params, searchParams }: { params: Promise<{ batchId: string }>; searchParams: Promise<{ committed?: string }> }) {
  ...
  return <main className="shell py-8">
    ...
    {query.committed ? <div className="mt-5 rounded-md border border-success/30 bg-success/10 p-3 text-sm">Import completed. Searchable profiles are now live.</div> : null}
    <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <Card><p className="text-sm text-muted-foreground">New profiles</p><p className="mt-2 text-3xl font-semibold">{batch.newLocationCount}</p></Card>
    ...
    <div className="mt-5 overflow-x-auto rounded-lg border border-border"><table className="w-full min-w-[1000px] text-left text-sm"><thead className="bg-muted/50"><tr><th className="p-3">Row</th><th className="p-3">Organization</th><th className="p-3">Program</th><th className="p-3">Delivery / coverage</th><th className="p-3">Address</th><th className="p-3">Result</th><th className="p-3">Errors / message</th></tr></thead><tbody>
      {batch.rows.map((row) => { const raw = row.rawData as Record<string, string>; const normalized = row.normalizedData as unknown as NormalizedProviderImportRow | null; const virtual = normalized?.telehealthMode === "Virtual only"; return <tr className="border-t border-border align-top" key={row.id}><td className="p-3">{row.rowNumber}</td><td className="p-3">{raw.organization_name}</td><td className="p-3 font-medium">{raw.program_name}</td><td className="p-3">{normalized?.telehealthMode || raw.delivery_mode || "In-person / existing mode"}{virtual ? <><p className="mt-1 max-w-xs text-xs">{coverageLabel(normalized?.statesServed || [])}</p><p className="mt-1 text-xs">{normalized?.programmingTimeZone}</p></> : raw.states_served ? <p>{raw.states_served}</p> : null}</td><td className="p-3">{virtual ? "Not applicable" : [raw.address, raw.city, raw.state, raw.zip].filter(Boolean).join(", ")}</td><td className="p-3"><Badge tone={row.status === "rejected" || row.status === "failed" || row.previewAction.includes("update") ? "warning" : "success"}>{row.previewAction}</Badge></td><td className="max-w-md p-3 text-sm">{row.errorReasons.length ? row.errorReasons.join(" ") : row.resultMessage || "Ready to commit."}</td></tr>; })}
    </tbody></table></div>
  </main>;
}
